...
public class MemberResourceRESTService {
	 ...
	 * @Path("/{id:[0-9][0-9]*}")
	 * 
	 * @Produces(MediaType.APPLICATION_JSON) public Member
	 * lookupMemberById(@PathParam("id") long id) { Member member =
	 * repository.findById(id); if (member == null) { throw new
	 * WebApplicationException(Response.Status.NOT_FOUND); } return member; }
	 * 
	 *//**
	 ...
	 * 
	 * @Produces(MediaType.APPLICATION_JSON) public Response createMember(Member
	 * member) {
	 * 
	 ...
	 * 
	 * // Create an "ok" response builder = Response.ok(); } catch
	 * (ConstraintViolationException ce) { // Handle bean validation issues builder
	 * = createViolationResponse(ce.getConstraintViolations()); } catch
	 * (ValidationException e) { // Handle the unique constrain violation
	 * Map<String, String> responseObj = new HashMap<>(); responseObj.put("email",
	 * "Email taken"); builder =
	 * Response.status(Response.Status.CONFLICT).entity(responseObj); } catch
	 * (Exception e) { // Handle generic exceptions Map<String, String> responseObj
	 * = new HashMap<>(); responseObj.put("error", e.getMessage()); builder =
	 ...
	 * validator and check for issues. Set<ConstraintViolation<Member>> violations =
	 ...
	 *//**
		 ...
		 *//*
			 * public boolean emailAlreadyExists(String email) { Member member = null; try {
			 * member = repository.findByEmail(email); } catch (NoResultException e) { //
			 * ignore } return member != null; }
			 */
}
